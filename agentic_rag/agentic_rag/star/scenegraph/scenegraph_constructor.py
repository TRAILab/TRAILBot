...
from agentic_rag.star.utils.utils import (
    ...
    load_models,
    # get_observation_by_window,
    gobs_to_detection_list_2,
    denoise_objects,
    accumulate_pc,
    distance_filter,
    load_calib,
...
def run_scenegraph_generation(
    ...
    try:
        for data_in_window in iterator(data_source, loop_event):
            image, pc, pose, his_pcs, his_poses, timestamp = data_in_window
            ...
            if his_pcs is not None:
                pc = accumulate_pc(cfg, mos_model, pc, pose, his_pcs, his_poses) # Accumulate and filter dynamic objects
            pc = pc[pc[:, 2] <= 4.0] # Filter out points that are too high
            # Filter out points that are too far
            if cfg.filter_dis:
                pc = distance_filter(cfg.max_depth, pc)

            # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            pro_point_new, pixels_new, z_new = project(pc, image, calib)
            save_path = Path(os.path.join(cfg.save_vis_path, f"vis_{file_names}")).with_suffix(".png")
            if not save_path.parent.exists():
                os.makedirs(save_path.parent)
            # cv2.imwrite(save_path, fusion)

            # print(f"project time: {t3-t2:.2f}s")
            
            if cfg.back_projection:
            # Plot the image
                import matplotlib.pyplot as plt
                duplicate = image.copy()
                duplicate = cv2.cvtColor(duplicate, cv2.COLOR_BGR2RGB)
                ...
                plt.scatter(x_pix, y_pix, c=pro_point_new[:, 2], cmap='jet', s=1)
                plt.scatter(x_pix, y_pix, c=z_new, cmap='jet', s=1)
            ...
            detection_list, bg_list, _, _, valid_mask_indices = gobs_to_detection_list_2(
                cfg = cfg,
                image = bgr_image,
                pc = pro_point_new,
                pixels = pixels_new,
            ...
            #         pcd = objects[iii]['pcd']  # already an Open3D PointCloud
            #         os.makedirs(f"/home/trailbot/RAG/results/{cfg.sequence}/detection_objects/{idx}/", exist_ok=True)
            #         o3d.io.write_point_cloud(f"/home/trailbot/RAG/results/{cfg.sequence}/detection_objects/{idx}/output_{iii}.ply", pcd)
            ...
            #         start_sphere.paint_uniform_color(color)  # Red color
            #         selected_pos.append(start_sphere)
            #         print(f"start_point: {selected_pos}; current pose: {pose[:3, 3]}")
            #         visualization_objects = point_clouds + selected_pos
            #         o3d.visualization.draw_geometries(visualization_objects)
            # print(f"idx: {idx}, time: {time.time() - time_1:.2f} seconds")
