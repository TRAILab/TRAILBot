...
from agentic_rag.star.agents.agent_sg import Agent, AgentOutput
...
def should_continue_aib(state: AgentState):
    messages = state["messages"]
...
class ReMEmbRAgent_SG(Agent):
    ...
    def __init__(self, llm_type='gpt-4o', num_ctx=8192, temperature=0):
        ...
        self.agent_prompt = file_to_string(top_level_path+'prompts_isaacsim/agent_system_prompt_sg.txt')
    ...
    def llm_selector(self, llm_type, temperature, num_ctx):
        ...
        elif 'gpt-oss:20b' in llm_type:
            print("Using GPT-OSS 20B LLMs!")
    ...
    def create_tools(self, memory):
        ...
        self.tool_list = [self.scenegraph_retriever_tool, self.nearest_object_tool] # , self.scenegraph_retriever_tool
        self.tool_definitions = [convert_to_openai_function(t) for t in self.tool_list]
    ...
    def agent(self, state):
        ...
        if ('gpt-4' not in self.llm_type) and ('nim' not in self.llm_type) and ('o4' not in self.llm_type) and ("gpt" not in self.llm_type):
    ...
    def query_aib(self, question: str):
