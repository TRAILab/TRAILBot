...
class ReMEmbRAgent(Agent):
    ...
    def set_memory(self, memory: Memory, scene_graph=None, dataset_start_timestamp=None, sbert_model=None):
        self.memory = memory
    ...
    def create_tools(self, memory):
        ...
        self.tool_list = [self.retriever_tool, self.position_retriever_tool, self.time_retriever_tool, self.scenegraph_retriever_tool, self.nearest_object_tool]
        self.tool_definitions = [convert_to_openai_function(t) for t in self.tool_list]
    ...
    def agent(self, state):
        ...
        print(colored(str({"messages": [response]}), "blue", attrs=["bold"]))
