...
import spacy
from agentic_rag.star.some_class.map_class_isaac import MapObjectList, DetectionList
...
def group_objects(pcd: o3d.geometry.PointCloud, eps=0.01, min_points=10) -> List[o3d.geometry.PointCloud]:
    '''
    group objects using DBSCAN clustering
    '''
...
def prepare_point_cloud_data(objects, all_points, point_to_object_map):
    """Extract point cloud data and map it to object indices."""


    for idx, obj in enumerate(objects):
        # Check if the 'pcd' key exists and contains valid points
        if 'pcd' in obj and hasattr(obj['pcd'], 'points'):
            points = np.asarray(obj['pcd'].points)  # Ensure points is a NumPy array
            if points.size > 0:  # Ensure points has at least one point
                all_points.append(points)
                point_to_object_map.extend([idx] * points.shape[0])
            else:
                print(f"Object {idx} has no points. Skipping.")
        else:
            print(f"Object {idx} is missing a valid 'pcd' or 'pcd.points'. Skipping.")
    
    if all_points:
        return np.vstack(all_points), point_to_object_map
    else:
        return np.array([]), point_to_object_map

# Step 2: Incremental clustering using DBSCAN and k-d tree
def incremental_cluster(existing_clusters, new_points, all_points, eps=3.0, min_samples=2):
    """Incrementally cluster new points and update existing clusters."""
    # Compute cluster centers (skip empty clusters)
    cluster_centers = []
    valid_cluster_ids = []
    for cluster_id, cluster_points in existing_clusters.items():
        if len(cluster_points) > 0:
            cluster_centers.append(np.mean(all_points[cluster_points], axis=0))
            valid_cluster_ids.append(cluster_id)

    if cluster_centers:
        cluster_centers = np.vstack(cluster_centers)  # Ensure 2D array
        cluster_index = cKDTree(cluster_centers)
    else:
        cluster_centers = np.empty((0, all_points.shape[1]))
        cluster_index = cKDTree(cluster_centers)

    # Cluster new points
    dbscan = DBSCAN(eps=eps, min_samples=min_samples)
    new_labels = dbscan.fit_predict(new_points)
    new_clusters = defaultdict(list)
    for point_idx, label in enumerate(new_labels):
        if label == -1:
            continue  # Ignore noise
        new_clusters[label].append(point_idx)
    
    # Merge new clusters into existing clusters
    for new_cluster_id, new_cluster_points in new_clusters.items():
        merged_points = set(new_cluster_points)
        if cluster_centers.shape[0] > 0:
            nearby_clusters = cluster_index.query_ball_point(
                np.mean(new_points[new_cluster_points], axis=0), r=eps
            )
            for cluster_id in nearby_clusters:
                merged_points.update(existing_clusters[valid_cluster_ids[cluster_id]])
                del existing_clusters[valid_cluster_ids[cluster_id]]  # Remove old cluster
        
        new_cluster_id = len(existing_clusters)
        existing_clusters[new_cluster_id] = list(merged_points)

    return existing_clusters

# def incremental_cluster(existing_clusters, new_points, all_points, eps=3.0, min_samples=2):
#     """Incrementally cluster new points and update existing clusters."""
#     # Build a k-d tree for existing clusters
#     cluster_centers = [
#         np.mean(all_points[cluster], axis=0) for cluster in existing_clusters.values()
#     ]
#     cluster_index = cKDTree(cluster_centers)
...
#         )
#         for cluster_id in nearby_clusters:
#             merged_points.update(existing_clusters[cluster_id])
#             del existing_clusters[cluster_id]  # Remove old cluster
...
# Step 3: Group objects by clusters
def group_objects_by_clusters(clusters, point_to_object_map, objects):
    """Group objects based on clusters."""
    grouped_captions = {}
    for cluster_id, cluster_points in clusters.items():
        object_indices = list(set([point_to_object_map[idx] for idx in cluster_points]))
        grouped_captions[cluster_id] = {
            "objects": object_indices,
            "captions": [objects[idx]['caption'] for idx in object_indices if 'caption' in objects[idx]],
        }
    return grouped_captions

# Step 4: Summarize grouped captions
def summarize_grouped_captions(grouped_captions):
    """Summarize captions for each cluster."""
    sbert_model = SentenceTransformer('all-MiniLM-L6-v2')
    summaries = {}
    for cluster_id, data in grouped_captions.items():
        captions = " ".join(data["captions"])
        if captions.strip():
            prompt = f"Summarize the following descriptions:\n{captions}"
            summary = f"Cluster {cluster_id}: {captions}"  # Simulate summarization
            summaries[cluster_id] = summary
        else:
            summaries[cluster_id] = "No meaningful captions available."
    return summaries

# Step 5: Update SceneGraph
def update_scene_graph(scene_graph, clusters, grouped_captions, summaries):
    """Update the scene graph with new clusters and summaries."""
    for cluster_id, data in grouped_captions.items():
        scene_graph.add_node(cluster_id, data["objects"], summaries[cluster_id])
