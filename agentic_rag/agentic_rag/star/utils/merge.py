...
from agentic_rag.star.some_class.map_calss import DetectionList, MapObjectList
...
import time
from agentic_rag.star.utils.utils import get_bounding_box, process_pcd
...
def compute_iou_batch(bbox1: torch.Tensor, bbox2: torch.Tensor) -> torch.Tensor:
    ...
    # iou_1 = inter_vol / (bbox1_vol + 1e-10)  # Shape: (M, N)
    # iou_2 = inter_vol / (bbox2_vol + 1e-10)  # Shape: (M, N)
    # g_iou = torch.max(iou_1, iou_2)  # Shape: (M, N)
    # print(inter_vol)
    # print(iou)
    # print(bbox1)
    # print(bbox2)
...
def aggregate_similarities(cfg, spatial_sim: torch.Tensor,  ft_similarities: torch.Tensor, caption_similarities: torch.Tensor) -> torch.Tensor:
    ...
    if caption_similarities is not None:
        ...
        # print(f"spatial_sim:\n {spatial_sim}, caption_similarities:\n {caption_similarities}, ft_similarities: {ft_similarities}")
    else:
...
def merge_detections_to_objects_2(
    ...
    for i in range(agg_sim.shape[0]):
        if agg_sim[i].max() == float('-inf'):
            ...
            print(
                f"Adding new detection list \033[96m{valid_mask_indices[i]},\033[0m as map object {new_obj_idx} "
            ...
            print(
                f"Merging detected object from\033[94m detection list {valid_mask_indices[i]},\033[0m "
