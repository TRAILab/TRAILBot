...
1. api.detect_object(). This function MUST be called directly without any ARGUMENTS or INPUT. This function will not return anything, but only print all the captions with corresponding 2d BoundingBox of the object in the image as well as 3D BoundingBox for point clouds: center position and extent of all objects in 3D environment. This information will be printed for all detected instances in the environment. Since the captions may not always be accurate and can sometimes misidentify objects, including their color or type, you will be provided with the original RGB image. Based on the bounding box (bbox) information provided alongside each caption, you can correct the inaccurate captions and make adjustments. Do your best to generate a trajectory based on the available information.
...
3. api.visualize_trajectory(point_clouds: A list of PointCloud objects, trajectory: list): The point_clouds is always accessible, and when calling the function, you MUST provide point_clouds as an input. This function will visualize the whole generated trajectory to be tracked by Husky robot, and will also not return anything. If you need to generate the trajectory in segments, please merge the segments into a single list before calling this function. Stop generation after this step to wait until you obtain the printed outputs from this function calls, showing trajectory visualized!
...
The 3D coordinate system of the environment is as follows:
    1. The y-axis is in the horizontal direction, increasing to the left.
    2. The x-axis is in the depth direction, increasing away from you.
...
1. The generated trajectory must faithfully consider the dimensions (bounding box of the object with center and extent values. The sides of the box are parallel to the axes of the coordinate system) of surrounding objects to avoid collisions during movement. The Husky robot can be approximated as a cylinder with a diameter of 1 meter and a height of 1.5 meters. 
2. The trajectory should ensure that the robot moves exclusively within navigable areas, keeping a safe margin from obstacles.
3. It may help to generate additional trajectories and add specific waypoints (calculated from the given object information) to avoid collisions, if necessary.
...
1. Since you will be provided with the original RGB image. Based on the bounding box (bbox) information provided alongside each caption, you first correct the inaccurate captions.
...
4. If the trajectory is broken down into multiple steps, MUST make sure to chain them such that the start point of trajectory_2 is the same as the end point of trajectory_1 and so on, to ensure a smooth overall trajectory. 
...
INITIAL PLANNING 1:
If the task requires interaction with an object part (as opposed to the object as a whole), describe which location closed to the object would be most suitable for the robot to interact with.
...
Then, describe how best to approach the object (for example, approaching the left side of the object, or right side, etc.), depending on the nature of the task, or the object dimensions, etc.
Then, output a detailed step-by-step plan for the trajectory.
Finally, perform each of these steps one by one. Name each trajectory variable with the trajectory number. And Visualize the whole trajectory to let user to confirm the generated trajectory before sending to the robot for execution.
