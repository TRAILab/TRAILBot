...
from utils.helpers import visualize_map
import copy
import platform
import multiprocessing
import logging
    ...
    while True: # todos
        ...
        if not observation_buffer.empty():
            ...
            # Plot the image
                ...
                save_path = "/home/trailbot/Documents/data_process_Kitti/results/auto_compensate/"
                # Show the plot
            ...
            api = API(configs, detection_list, bg_list, pose, used_idx, logger, sematic_info, drivable_indices, robot_state, sematic_map, configs.resolution, map_origin, occupancy_map, terrain_map)
            ...
            if configs.vis_all: #observation_buffer.empty() and     and used_idx % 2 == 0 and used_idx>2
                if VLT:
                    ...
                    #command = "go to the board that says 'caution', the one with small cardboard box"#closer to you " #yellow bollard with some wooden pallets near it" #"go to the stop sign" #board that says 'caution'" #"move forward for 2 meters" # #"fan on top of the machine" #board that says 'caution'" #"go to the chair with a yellow box on it" # "go to the board that says 'caution'" #"go to the ladder" #input("Enter the command: ")
                    # "go to the ladder" #"Go to the red bag"
                    ...
                    messages = models.get_chatgpt_output("gpt-4o", new_prompt, messages, "system", command, image_path1)
                    ...
                    while not api.completed_task:
                        ...
                        if not api.completed_task:
                            if api.failed_task:
                                ...
                                messages = models.get_chatgpt_output("gpt-4o", new_prompt, messages, "user", command, image_path1)
...
class SceneGraph(Node):
    ...
    def __init__(self, cfg, observation_buffer, sync_queue_size: int = 1000, max_time_diff: float = 0.1):
        self.configs= cfg
    ...
    def observation_callback(self, camera_msg: Image, lidar_msg: PointCloud2, tf_msg: PoseStamped) -> None:
        ...
        print("Observation callback times:", self.callback_times)
...
if __name__ == '__main__':
    ...
    command2 = "ros2 bag play /home/trailbot/bags/ptp_timeX" #2Indoor9 Outdoor
