...
def main(cfg : DictConfig):
    ...
    current_poses = [[row[0], row[1]] for row in current_poses]
    ...
    for current_pose in current_poses:
        x, y = current_pose
        # 使用KD树查询半径20m内的所有点
        near_indices = kd_tree.query_radius(np.array([x, y]).reshape(1, -1), r=2)[0]
    ...
    # 所有道路结点
    print(f"unique_cluster_centers shape: {unique_cluster_centers.shape}")
    if isinstance(duandians, list):
        duandians = np.array(duandians)
    print(f"duandians shape: {duandians.shape}")
    ...
    for i in range(len(all_path_nodes)):
        indexes = np.where(distances[i] < 5)[0]
    ...
    nodes, edges = refine_nodes(all_path_nodes, paths_index, dis=5)
