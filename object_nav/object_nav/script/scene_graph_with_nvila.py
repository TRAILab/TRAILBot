...
#Nvila related
from utils.nvila_captioner import NVILACaptioner
...
BG_CAPTIONS_Pro_Sim = ["paved road", "paved road", "paved road", "sidewalk", "sidewalk", "sidewalk", "sidewalk", "floor"]#
...
def run_scenegraph_generation(configs, observation_buffer, objects, mask_generator, calib):
    ...
    parser2.add_argument("--seq_id", type=int, default=0)
    parser2.add_argument("--data_path", type=str, default="/home/trailbot/workspace/VILA/dataset")
    parser2.add_argument("--seconds_per_caption", type=int, default=0.2)
    parser2.add_argument("--out_path", type=str, default="/home/trailbot/workspace/VILA/dataset/captions_whole")
    parser2.add_argument("--num-video-frames", type=int, default=6)
    parser2.add_argument("--captioner_name", type=str, default="Nvila_lite_2b")

    parser2.add_argument("--model-path", type=str, default="Efficient-Large-Model/NVILA-Lite-2B")
    parser2.add_argument("--conv-mode", "-c", type=str, default="auto")
    # parser2.add_argument("--query", type=str, default="\
    #                 The output json format is as follows: \
    #                     {\
    #                     'scene': describe the video in detail,\
    #                     'location': choose from indoor or outdoor}")
    parser2.add_argument("--query", type=str, default="describe the scene in detail, including the objects, people, and environment.")
    parser2.add_argument("--text", type=str)
    # parser2.add_argument("--media", type=str, nargs="+", default= [f"{base_path}{img}{200}.png", f"{base_path}{img}{400}.png", f"{base_path}{img}{800}.png"])#["/home/mfyuan/local_folder/OpenNav_v2/deps1/VILA/demo_images/cam0.png"])
    parser2.add_argument("--media", type=str, default=["./file_example_MP4_480_1_5MG.mp4"])#/media/ssd/Local_data/CODa_dataset/videos/0/output.mp4
    parser2.add_argument("--json-mode", action="store_true")
    parser2.add_argument("--json-schema", type=str, default=None)
    args2 = parser2.parse_args()
    
    embedder = HuggingFaceEmbeddings(model_name='mixedbread-ai/mxbai-embed-large-v1')
    #NVILA part

    nvila_model = NVILACaptioner(args2)
    ...
    while True: # todos
        if not observation_buffer.empty():
            ...
            # if True:
            if use_nvila and used_idx % 5 == 0:
                captions_location = args2.out_path
                # if os.path.exists(captions_location):
                if False:
                    print(f"Captions already exist in {captions_location}. Exiting.")
                    # exit()
                    # shutil.rmtree(captions_location, ignore_errors=True)
                else:
                    os.makedirs(captions_location, exist_ok=True)

                    #randamly generate the position and rotation of the camera -- will changed to Husky data later
                    position = np.random.rand(3)  
                    rotation = np.random.rand(3)  
                    timestamp = np.random.rand(1)
                    max_retries = 1
                    retry_delay = 1.0  # seconds
                    output = []
                    for attempt in range(max_retries):
                        out_text = nvila_model.caption(image)

                        # Optionally sanitize control characters before parsing
                        out_text_clean = re.sub(r"[\x00-\x1F]+", "", out_text)

                        # Try to parse
                        # out_text_dict = json.loads(out_text_clean)

                        # Extract scene and location safely
                        # scene = out_text_dict.get("scene", "").replace("\n", " ").strip()
                        # location = out_text_dict.get("location", "").strip()

                        # print(colored(scene, "cyan", attrs=["bold"]))
                        # print(colored(location, "yellow", attrs=["bold"]))
                        print(colored(out_text, "yellow", attrs=["bold"]))
                    ...
                    entity = {
                        'id': "TBD",  # TEMPORARY: We are not using the id yet, so just leaving a placeholder
                    ...
                    print("\033[96mFinishing NVILA\033[0m" + "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                    # with open(os.path.join(captions_location, f'captions_{args2.captioner_name}_{args2.seconds_per_caption}_secs.json'), 'w') as f:
            ...
            # Plot the image
                ...
                # Show the plot\
                os.makedirs("/home/trailbot/Documents/data_process_Kitti/results/july4/", exist_ok=True)
                plt.savefig(f"/home/trailbot/Documents/data_process_Kitti/results/july4/saved_{frame_id}.png", dpi=300, bbox_inches='tight')
            p3 = time.time()
...
class SceneGraph(Node):
    ...
    def __init__(self, cfg, observation_buffer, sync_queue_size: int = 1000, max_time_diff: float = 0.05):
        ...
        self.tracked_pose_sub = message_filters.Subscriber(self, PoseStamped, '/tracked_pose')
    ...
    def observation_callback_tracked(self, camera_msg: Image, lidar_msg: PointCloud2, tracked_pose_msg: PoseStamped) -> None:
        """
        Callback function for processing camera and lidar messages and pose message.
        ...
        """
        t1 = time.time()
        print("Observation callback triggered")
        cv_image = self.bridge.imgmsg_to_cv2(
            camera_msg, desired_encoding='rgb8')#rgb8
        cv_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2RGB) #Added for colour correction to RGB
        self.cv_image = cv2.undistort(cv_image, self.camera_transformation_k, self.radial_distortion, None, self.new_K)
        ...
        # translation = tf_msg.transform.translation
        # print(type(tf_msg))
        # print(type(tracked_pose_msg))
        # print("SUCCCECSS")
        # translation = tracked_pose_msg.transform.translation
        # print("SUCCCECSS")
        # exit(0)
        pose = tracked_pose_msg.pose
        ...
        self.observation_ready = True
        self.callback_times += 1
        # print(f"image timestamp:{self.timestamp_cam}\nlidar timestamp:{self.timestamp_lidar}\npose: {tracked_pose_msg.header.stamp}")
        # print(f"before ready: {time.time()}")
        # exit(0)
        t4 = time.time()
        #print(f'time for processing one pair of data: {t4-t1} with camera: {t2-t1}, lidar: {t3-t2} and transformation: {t4-t3}')  
    ...
    def run_livedata_processing(self):
        ...
        while rclpy.ok():
            ...
            if self.observation_ready:
                ...
                lidar_2map_TF = base_link_2map_TF
...
if __name__ == '__main__':
    ...
    command2 = "ros2 bag play /home/trailbot/bags/ros2_time" #2Indoor9
