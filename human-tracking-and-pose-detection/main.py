...
parser.add_argument('-v', '--verbose', action='store_true', help='Enable print_verbose_only outputs')
parser.add_argument('-k', '--display_keypoint', action='store_true', help='Enable display of keypoints')
parser.add_argument('-i', '--display_image', action='store_true', help='Enable display of image and keypoints')
...
def print_verbose_only(*args, **kwargs):
  ...
  if parser_args.verbose: print(*args, **kwargs)
...
#movenet model. Gets input image and outputs array of keypoints with certainty score
#downloaded from #https://tfhub.dev/google/movenet/multipose/lightning/1
model = hub.load("models_downloaded/multipose_tf").signatures['serving_default'] 
...
#get the heading angle of the person, in degree, relative to the center of the field of view
def getHeadingAngle(points,score_threshold=0.3, fov=90, image_width=1, offset=0, scaling=1): 
  visible_points = points[ points[:,2] > score_threshold]
  x_mean = np.mean(visible_points[:, 1])
  x_angle_radian = math.atan((x_mean - (image_width/2)) / (image_width/2) * math.tan( math.radians(fov/2) ) )
  return offset + scaling * math.degrees(x_angle_radian)

#draw keypoints on a black image
def keypoints_to_img(points, nth_point_to_highlight=None, score_threshold=0.2, img_size=128):
...
#process a frame. Determine keypoints and number of people and heading angle.
@print_fps
...
def main():
  ...
  while True:
    ...
    keypoints = process_frame(image)

    # Visualize the predictions keypoints only
    if parser_args.display_keypoint:
      output_frame = keypoints_to_img(keypoints, nth_point_to_highlight=1)
    ...
    if parser_args.display_image:
      ...
      output_overlay = draw_prediction_on_image(np.squeeze(display_image.numpy(), axis=0), keypoints.reshape((1, 1, 17, 3)))
