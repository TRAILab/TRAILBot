...
class TFBufferListener(Node):
    def __init__(self):
        super().__init__('tf_buffer_listener')
        print("INIT")
...
def main(args=None):
    ...
    try:
        # Query transforms periodically until the bag is done or the node shuts down
        while bag_thread.is_alive() and rclpy.ok():
            node.query_transform()
    ...
    print("Finished processing transformations.")

    # Save and plot data
    data = node.save_data()
    data.to_csv('map_tf2.csv', index=False)
    print(data)
