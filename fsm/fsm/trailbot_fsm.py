...
class FSM(Node):
  ...
  def target_callback(self, msg):
    try:
      ...
      # Print the coordinates of the new target point
      #self.get_logger().info(
      #      f"New target point: x={new_target_point.pose.position.x}, "
      #      f"y={new_target_point.pose.position.y}, "
      #      f"z={new_target_point.pose.position.z}"
      #  )
      # self.blackboard["target_found"] = False
  ...
  def trail_callback(self, msg):
    try:
      ...
      if self.blackboard['trail_pose'] is not None:
        ...
        delta = np.array([old_trail_point.pose.position.x-old_trail_point.pose.position.x, \
                          new_trail_point.pose.position.y-new_trail_point.pose.position.y])
